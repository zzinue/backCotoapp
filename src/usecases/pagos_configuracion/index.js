...
const get = async () => {
    ...
    return await Pagos_configuracion.find().populate("pagos_configuracion").exec();

}

const getById = async (id) => {
    const pagos_configuracion =  await Pagos_configuracion.findById(id).populate("pagos_configuracion").exec();
...
const update = async (id, Pagos_configuracionData) => {
    ...
    const { 
        concepto,descripcion,monto,periodicidad,tieneFechaLimite,fechaLimite,fecha_registro, permisos } = Pagos_configuracionData;
